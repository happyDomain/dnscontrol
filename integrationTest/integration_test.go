...
func openpgpkey(name, target string) *models.RecordConfig {
	name += "._openpgpkey"
	r := makeRec(name, target, "OPENPGPKEY")
	r.SetTargetOpenPGPKey(target)
	return r
}
...
func makeTests() []*TestGroup {
	...
	tests := []*TestGroup{
		...
		testgroup("CAA",
			...
			tc("CAA issuemail", caa("@", "issuemail", 0, ";")),
