...
func generateTypeFile(t *TypeDef) error {
	...
	if anyNeedsNetip(t.Fields) || anyNeedsNetip(t.OptionalFields) || anyNeedsNetip(t.RuntimeFields) {
	...
	for i, f := range t.OptionalFields {
		ti := info(f.Type)
...
func generateRdataFile(t *TypeDef) error {
	...
	third = append(third, `"github.com/DNSControl/dnscontrol/v5/pkg/nrc"`)
	// }
	if needsTxtutil(t.Fields) || needsTxtutil(t.OptionalFields) || needsTxtutil(t.RuntimeFields) {
